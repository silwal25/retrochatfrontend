        ...
        switch (action.type) {
            case "usernameImmediately":
                ...
                if (draft.username.value.length > 30) {
                    ...
                    draft.username.message = "Username should be less than 30 characters"
                }
                if (draft.username.value && !/^([a-zA-Z0-9_]+)$/.test(draft.username.value)) {
                    draft.username.hasErrors = true
                    draft.username.message = "Username can only contain letters, numbers and underscore"
                }
                break
            case "usernameAfterDelay":
                if (draft.username.value.length < 3) {
                    draft.username.hasErrors = true
                    draft.username.message = "Username must be atleast 3 characters long"
                }
            ...
            case "passwordAfterDelay":
                if (draft.password.value.length < 12) {
                    draft.password.hasErrors = true
                    draft.password.message = "Password must be atleast 12 characters long"
                }
                break
            case "submitForm":
        ...
        if (state.submitCount) {
            ...
            async function fetchResults() {
                try {
                    ...
                    appDispatch({ type: "flashMessage", value: "Successfully registered" })
                } catch (e) {
                    console.log("error")
                }
            }
                ...
                <div className="col-lg-5 pl-lg-5 pb-3 py-lg-5">
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className="form-group">
                            ...
                            <input onChange={(e) => dispatch({ type: "emailImmediately", value: e.target.value })} id="email-register" name="email" className="form-control" type="text" placeholder="you@example.com" autoomplete="off" />
